...
void iterate_page_rank(int size, double curr_page_rank[size], 
double prev_page_rank[size]);
double calculate_diff(int size, double curr_page_rank[size], 
double prev_page_rank[size]);
double weighted_formula(char *url, Graph web, int size, Set urls, 
double curr_page_rank[size]);
double out_links(char *url, Graph web, Set urls);
double in_links(char *url, Graph web, Set urls);
double weighted_value(char *prev_url, Graph web, int size, Set urls, 
double page_rank, char *url);
...
int main (int arg , char *argv[]) {
	...
	while (iteration < maxIterations && diff >= diffPr) {
		...
		while (i < size) {
			...
			curr_page_rank[i] = ((1.0-d)/(double)size) + 
			(d * weighted_formula(url, web,size, urls, prev_page_rank));
